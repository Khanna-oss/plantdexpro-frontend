...
export const HealthBenefits = ({ plant }) => {
  ...
  const benefitChips = plant.benefitChips || ["Analyzed Profile"];
    ...
    <section className="space-y-16">
      <div className="flex items-center justify-between">
        ...
        <div className="bg-[#8b5a2b]/10 border border-[#8b5a2b]/20 px-6 py-2 rounded-full flex items-center gap-3">
          ...
          <span className="text-[10px] font-black text-[#4a3728] dark:text-[#f4f1ea] uppercase tracking-widest">Verified: {confidence}%</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
        {[
          { icon: HeartPulse, label: 'Primary Vitamins', data: nutrients.vitamins },
          { icon: Activity, label: 'Mineral Matrix', data: nutrients.minerals },
          { icon: Leaf, label: 'Phytochemicals', data: nutrients.proteins || 'Plant-based' }
        ].map((item, idx) => (
          <div key={idx} className="p-10 rounded-[2.5rem] bg-white dark:bg-gray-800 border-b-4 border-[#8b5a2b]/30 shadow-lg hover:shadow-2xl transition-all group">
            <div className="w-14 h-14 bg-[#8b5a2b]/10 rounded-2xl flex items-center justify-center mb-8 text-[#8b5a2b] group-hover:rotate-12 transition-transform">
               <item.icon size={28} />
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 pt-10">
        <div className="space-y-8">
          ...
          <div className="space-y-6">
            {healthHints.map((hint, idx) => (
              <div key={idx} className="p-8 rounded-[2.2rem] bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 flex gap-6 items-start shadow-sm hover:border-[#8b5a2b]/30 transition-all">
        ...
        <div className="space-y-8">
           <h4 className="text-[11px] font-black text-gray-400 uppercase tracking-[0.4em] mb-4">Safe Protocol</h4>
           <div className="p-12 rounded-[3.5rem] bg-[#4a3728] text-white shadow-xl relative overflow-hidden group min-h-[350px] flex flex-col justify-center">
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-5 mb-8">
                  ...
                  <span className="text-[13px] font-black uppercase tracking-[0.4em]">Preparation</span>
                </div>
