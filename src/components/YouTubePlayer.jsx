...
import { Play, Loader2, Youtube, ExternalLink, ImageOff, Sparkles, ChevronDown, ChevronUp } from 'lucide-react';
...
export const YouTubePlayer = ({ video, isActive, onPlay }) => {
  ...
  return (
    <div className="relative rounded-[2rem] overflow-hidden bg-gray-900/5 dark:bg-black/20 border border-white/20 dark:border-gray-800 group/player transition-all duration-500 hover:shadow-2xl">
      {isActive ? (
        ...
        >
          ...
          />
          <div className="absolute inset-0 bg-black/20 group-hover/player:bg-black/40 transition-colors flex items-center justify-center">
          ...
          <div className="absolute top-4 left-4">
             <div className="bg-black/60 backdrop-blur-md px-3 py-1 rounded-full border border-white/10 flex items-center gap-2">
                ...
                <span className="text-[8px] font-black text-white uppercase tracking-widest">AI Ranked High</span>
             </div>
          </div>
        </button>
      )}
      ...
      <div className="p-6">
        <h4 className="text-xs font-black text-gray-900 dark:text-white line-clamp-1 mb-2 tracking-tight group-hover/player:text-emerald-500 transition-colors">
          {video.title}
        </h4>
